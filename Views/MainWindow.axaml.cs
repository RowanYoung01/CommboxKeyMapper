...
public partial class MainWindow : Window
{
    ...
    private const string StartupAppName = "CommboxMapper";
    ...
    private void Init()
    {
        ...
        _leftSidetoneSlider.ValueChanged  += (_, _) => OnSidetoneVolumeChanged(left: true);
        _rightSidetoneSlider.ValueChanged += (_, _) => OnSidetoneVolumeChanged(left: false);
        ...
        // Always start hidden in tray (only on first open)
        Opened += (_, _) => { if (!_startupHideDone) { _startupHideDone = true; HideToTray(); } };
    ...
    private void InitTrayIcon()
    {
        ...
        if (name != null)
        {
            // Icon copies from stream immediately; safe to dispose after construction
            var stream = asm.GetManifestResourceStream(name)!;
            icon = new System.Drawing.Icon(stream, 32, 32);
            stream.Dispose();
        }
        else
        {
            icon = System.Drawing.Icon.ExtractAssociatedIcon(AppExePath) ?? System.Drawing.SystemIcons.Application;
        }

        var cm = new WF.ContextMenuStrip();
        cm.Items.Add(new WF.ToolStripMenuItem("Commbox Mapper") { Enabled = false, Font = new System.Drawing.Font("Segoe UI", 9f, System.Drawing.FontStyle.Bold) });
        ...
        _notifyIcon = new WF.NotifyIcon { Text = "Commbox Mapper", Icon = icon, Visible = true, ContextMenuStrip = cm };
    ...
    private void RefreshFromSettings()
    {
        ...
        _leftSidetoneSlider.Value  = Math.Clamp(_settings.LeftSidetoneVolume,  0, 100);
        _rightSidetoneSlider.Value = Math.Clamp(_settings.RightSidetoneVolume, 0, 100);
        _leftSidetoneLbl.Text  = $"{_settings.LeftSidetoneVolume} %";
        _rightSidetoneLbl.Text = $"{_settings.RightSidetoneVolume} %";
        UpdateConfigLabel(0);
        UpdateConfigLabel(1);
    }
    ...
    private void ProcessReport(byte[] report)
    {
        ...
        {
            byte b2 = report[2];
            if ((b2 & 0x01) != 0) AdjustSidetone(-3, left: true);   // B15 left knob CCW
            if ((b2 & 0x02) != 0) AdjustSidetone(+3, left: true);   // B16 left knob CW
            if ((b2 & 0x04) != 0) AdjustSidetone(-3, left: false);  // B17 right knob CCW
            if ((b2 & 0x08) != 0) AdjustSidetone(+3, left: false);  // B18 right knob CW
        }
    }

    private void AdjustSidetone(int delta, bool left)
    {
        int cur    = left ? _settings.LeftSidetoneVolume : _settings.RightSidetoneVolume;
        int newVol = Math.Clamp(cur + delta, 0, 100);
        if (newVol == cur) return;

        if (left) { _settings.LeftSidetoneVolume  = newVol; _leftSidetoneSlider.Value  = newVol; _leftSidetoneLbl.Text  = $"{newVol} %"; }
        else       { _settings.RightSidetoneVolume = newVol; _rightSidetoneSlider.Value = newVol; _rightSidetoneLbl.Text = $"{newVol} %"; }

        _sidetone.TrySetHardwareSidetone(true, Math.Max(_settings.LeftSidetoneVolume, _settings.RightSidetoneVolume));
        if (_deviceHandle != null && !_deviceHandle.IsInvalid)
            SidetoneEngine.TryHidSidetone(_deviceHandle, true, _settings.LeftSidetoneVolume, _settings.RightSidetoneVolume);
        Log($"{(left ? "Left" : "Right")} sidetone {newVol} %");
        _settings.Save();
    }
    ...
    private void ResetSidetone()
    {
        Log("Sidetone reset…");
        if (_deviceHandle != null && !_deviceHandle.IsInvalid)
            SidetoneEngine.TryHidSidetone(_deviceHandle, false, 0, 0);
        ApplySidetone();
    }

    private void ApplySidetone()
    {
        int lv = _settings.LeftSidetoneVolume, rv = _settings.RightSidetoneVolume;

        // 1. WASAPI device-topology path — unmutes the Windows-side audio node (global gate)
        bool hwOk = _sidetone.TrySetHardwareSidetone(true, Math.Max(lv, rv));
        if (!hwOk && _sidetone.LastError != null)
            Log($"Sidetone HW topology error: {_sidetone.LastError}");

        // 2. HID Report ID 5 — per-side volume in the ACU-624D firmware mixer
        bool hidOk = false;
        if (_deviceHandle != null && !_deviceHandle.IsInvalid)
            hidOk = SidetoneEngine.TryHidSidetone(_deviceHandle, true, lv, rv);

        string status = hidOk ? $"Sidetone active (L:{lv}% R:{rv}%)"
                      : hwOk  ? $"Sidetone active — topology only (L:{lv}% R:{rv}%)"
                      :          "Failed — check Commbox is connected";
        _sidetoneStatus.Text       = status;
        _sidetoneStatus.Foreground = (hwOk || hidOk) ? BrushGreen : BrushRed;
        Log($"Sidetone: {status}");
    }

    private void OnSidetoneVolumeChanged(bool left)
    {
        if (left)
        {
            int pct = (int)_leftSidetoneSlider.Value;
            _leftSidetoneLbl.Text          = $"{pct} %";
            _settings.LeftSidetoneVolume   = pct;
        }
        else
        {
            int pct = (int)_rightSidetoneSlider.Value;
            _rightSidetoneLbl.Text         = $"{pct} %";
            _settings.RightSidetoneVolume  = pct;
        }
        _settings.Save();
        _sidetone.TrySetHardwareSidetone(true, Math.Max(_settings.LeftSidetoneVolume, _settings.RightSidetoneVolume));
        if (_deviceHandle != null && !_deviceHandle.IsInvalid)
