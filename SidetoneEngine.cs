...
public sealed class SidetoneEngine : IDisposable
{
    ...
    public static bool TryHidSidetone(Microsoft.Win32.SafeHandles.SafeFileHandle handle,
                                      bool enable, float leftPct, float rightPct)
    {
        ...
        // Bytes 1-4 (usages 0x20-0x23) = left side, bytes 5-8 (0x24-0x27) = right side.
        static byte Atten(bool en, float pct) =>
            en ? (byte)Math.Clamp((int)((100f - pct) / 100f * 64), 0, 64) : (byte)64;

        byte la = Atten(enable, leftPct);
        byte ra = Atten(enable, rightPct);
        var report = new byte[9];
        report[0] = 0x05;
        for (int i = 1; i <= 4; i++) report[i] = la;
        for (int i = 5; i <= 8; i++) report[i] = ra;
